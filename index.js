...
app.get('/saveServer', (req, res) => {
    console.log(req.param('name'));
    console.log("el save Server")
    srvActual = req.param('name')
   console.log(srvActual)

})

app.post('/getImage', (req, res) => {
    console.log(req.body);
    console.log( "<img src='" + srvActual + '/' + req.body.fileName + "'" + "/>")
console.log(srvActual)
    var divFile = "<div>"
        + "<img src='" + srvActual + req.body.fileName + "'" + "/>"
        + "</div>"
...
function editImg(filePath, text) {
    ...
    Jimp.read(filePath)
        ...
        .then(function (font) {
            loadedImage.print(font, (wi * 20/ 100), (he *80/100), text)
                .write(filePath);
        })
        .catch(function (err) {
            console.error(err);
